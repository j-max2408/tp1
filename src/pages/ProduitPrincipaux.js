...
const ProduitPrincipaux = () => {
   ...
   {produitsPrincipaux.produits.map((item, index) =>{
    ...
    <img src={process.env.PUBLIC_URL + item.img} alt="" className="fluide"/>
    
    <h1 className="titre"><Link to={`/expositions/${item.nomProduit}`}> {item.Titre}</Link></h1>
    
   <h1 className="fleche">  <Link className="a-expos " to={`/expositions/${item.nomProduit}`} ><span>&#10547;</span></Link> </h1>
</article>
